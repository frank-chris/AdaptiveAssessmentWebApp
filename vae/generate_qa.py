...
args_checkpoint = 'save/vae-checkpoint/best_f1_model.pt'
device = torch.cuda.current_device()
checkpoint = torch.load(args_checkpoint, map_location="cpu")
...
class CustomDatset2(Dataset):
    def __init__(self, tokenizer, contexts, max_length=512):
        self.lines = contexts
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.num_total = len(self.lines)
...
def get_qa(contexts, tokenizer):
    args_max_length = 1150
    args_batch_size = 64
    
    args_k = 1

    data = CustomDatset2(tokenizer, contexts, args_max_length)
    data_loader = DataLoader(data, shuffle=False, batch_size=args_batch_size)
